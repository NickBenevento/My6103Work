...
# They are the "same" in that they contain the same values, but they are not the same object
# (i.e. do not map to the same place in memory).
...
#
print(f"i: Type {type(i)}, shape: {i.shape}, strides: {i.strides}")
print(f"j: Type {type(j)}, shape: {j.shape}, strides: {j.strides}")
print(f"idlabels: Type {type(idlabels)}, shape: {idlabels.shape}, strides: {idlabels.strides}")
# Output:
# i: Type <class 'numpy.ndarray'>, shape: (4, 3), strides: (24, 8)
# j: Type <class 'numpy.ndarray'>, shape: (4, 3), strides: (24, 8)
# idlabels: Type <class 'numpy.ndarray'>, shape: (2, 4, 3), strides: (96, 24, 8)
...
i[0, 0] = 8
print(f'i: {i}')
print(f'idlabels: {idlabels}')
# Changing i changes idlabels as well. This is what I would expect, since i was unpacked from 
# idlabels, so it maps to the same object in memory.
...
# write your codes here
#
nparray2c = np.copy(nparray2)
nparray2c[1, 1] = 0
print(f'nparray2c: {nparray2c}')
print(f'nparray2: {nparray2}')

# Only the copy was changed; this was unexpected, as I thought the original array
# would also change (due to the nature of shallow copies).
print(nparray2c == nparray2)
print(nparray2c is nparray2)
...
print(f'nparray2c: {nparray2c}')
print(f'nparray2: {nparray2}')
# The arrays remain separate, so they are not true copies (i.e. do not reference the same object).
print(nparray2c == nparray2)
print(nparray2c is nparray2)
